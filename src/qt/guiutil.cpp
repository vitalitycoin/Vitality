...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Vitality address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Vitality: URI
...
{
    // Convert Vitality:// to Vitality:
    //
    //    Cannot handle this later, because Vitality:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open Vitality.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Vitality.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Vitality.lnk
...
{
    return GetAutostartDir() / "Vitality.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Vitality.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Vitality\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Vitality app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Vitality app to startup item list
