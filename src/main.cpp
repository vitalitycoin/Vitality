...
#if defined(NDEBUG)
#error "Vitality cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0 * COIN;
    } else if (nHeight <= 75000 && nHeight > 500) {
        nSubsidy = 100 * COIN;
		...
		nSubsidy = 10 * COIN;
    } else if (nHeight > 500000 && nHeight <= 17154000) {
		nSubsidy = 5 * COIN;
    }else {
        nSubsidy = 0 * COIN;
    }
...
{
    RenameThread("Vitality-scriptch");
